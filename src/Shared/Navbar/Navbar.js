...
const Navbar = () => {
  ...
  useEffect(() => {
    axios
      .get(`${process.env.REACT_APP_ApiUrl}current/user?email=${user?.email}`, {
        headers: {
            ...
            <div>
              ...
              {userRole?.role === "publisher" && (
                <div>
                  ...
                  >
                    {user?.photoURL ? (
                      <FaUserCircle className="w-8 h-8 rounded-full border-red-400 border-2" />
                    ) : (
                      <FaUserCircle className="w-8 h-8 rounded-full border-red-400 border-2" />
                    )}
                  </div>
                  {checkRole && (
                    <div className="absolute right-0 bg-blue-500 w-[150px] p-3 rounded-tr-none rounded-lg text-white">
                      <h1>Profile </h1>
                ...
                <div>
                  ...
                  >
                    {user?.photoURL ? (
                      <img
                        src={user?.photoURL}
                        className="w-8 h-8 border-2 border-red-400 rounded-full"
                        alt=""
                      />
                    ) : (
                      <FaUserCircle className="w-8 h-8 rounded-full border-red-400 border-2" />
